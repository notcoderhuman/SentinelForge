...
DEFAULT_RULE_REGISTRY = RuleRegistry((
    ...
    RuleDefinition(
        "REPEATED_DNS_QUERY",
        "Repeated DNS query from hostname",
        "A hostname repeatedly queried the same DNS name within a bounded window.",
        "low", 120, True,
        ("five DNS query events for one hostname and query within 120 seconds",),
    ),
    RuleDefinition(
        "SOURCE_QUERIES_MANY_DOMAINS",
        "Hostname queried many distinct DNS names",
        "One hostname queried multiple distinct DNS names within a bounded window.",
        "medium", 120, True,
        ("five DNS query events for one hostname and five distinct queries within 120 seconds",),
    ),
    RuleDefinition(
        "DNS_QUERY_TO_SUSPICIOUS_DOMAIN",
        "DNS query matched suspicious local domain context",
        "A DNS query exactly matched a suspicious domain in local threat context.",
        "medium", 1, True,
        ("one DNS query whose domain exactly matches suspicious local threat context",),
    ),
    RuleDefinition(
        "DOMAIN_QUERIED_BY_MANY_SOURCES",
        "DNS query observed from many hostnames",
        "One DNS query was observed from multiple distinct hostnames within a bounded window.",
        "medium", 120, True,
        ("five DNS query events for one query from five distinct hostnames within 120 seconds",),
    ),
    RuleDefinition(
    ...
    RuleDefinition(
        ...
        "Repeated file activity",
        "The same file path was repeatedly modified within a bounded window.",
        "low", 120, True,
        ("five file activity events for one path within 120 seconds",),
    ),
    RuleDefinition(
        "HOST_MODIFIES_MANY_DISTINCT_FILES",
        "Host modifies many distinct files",
        "One host modified many distinct file paths within a bounded window.",
        "medium", 120, True,
        ("five file activity events for distinct paths on one host within 120 seconds",),
    ),
    RuleDefinition(
        "EXECUTABLE_FILE_CREATED",
        "Executable file created",
        "A file creation event targeted a conservatively identified executable path.",
        "medium", 1, True,
        ("one file creation event with an executable extension or executable directory",),
    ),
    RuleDefinition(
        "FILE_ACTIVITY_ON_SENSITIVE_PATH",
        "File activity on sensitive path",
        "File activity exactly matched a suspicious local file-path context.",
        "medium", 1, True,
